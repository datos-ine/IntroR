...
está listo para recibir instrucciones del usuario*.

Para separar expresiones en la misma línea se puede utilizar el símbolo
`;` (punto y coma), aunque se sugiere no hacerlo para evitar dificultades de lectura visual del código.
...
Los comandos u órdenes elementales de R son funciones construidas en
lenguaje C, otras provienen del Fortran y también existen muchas
escritas en el mismo lenguaje R.

Algunas de estas funciones se las llama "integradas" porque están
...
parámetros que deben ser escritos **entre paréntesis y separados por comas**. Incluso algunas de ellas que no tienen asociado argumentos
...
Cuando hacemos uso del lenguaje R es muy común encontrarse con errores y advertencias (warnings).

El lenguaje es muy preciso en su sintaxis y equivocarse en la forma de escribir una función o cualquier otro objeto produce respuestas de error del interprete de R
...
El que sea un lenguaje orientado a objetos nos lleva a entender la
lógica de funcionamiento pensada dentro de ese modelo de abstracción.

Podemos decir que todo con lo que trabaja R son objetos, y en esto
incluimos a las variables, los datos, las funciones, los resultados,
etc.

No vamos a profundizar en el tema pero necesitamos incorporar la idea y
el lenguaje cuando nos referimos a un objeto.

**Asignación**

Un grupos de objetos que veremos a continuación son las estructuras de
datos. Vamos a pensar a estas estructuras como "recipientes" con
determinada forma que van a contener valores (datos).
...
En este caso asignamos el valor 1 al objeto `a`, el valor 2 al objeto
`b` y el valor 3 al objeto `c`

Si llamasemos a cada objeto, el interprete nos devuelve el valor
guardado por cada uno de ellos.
...
Observemos que además de devolvernos los valores para cada uno de los
...
sola posición. (veremos este tema más adelante en tema [indexación](https://cballejo.github.io/R_Epi_Avanzada/Intro_R/index.html#índices))
...
También puede leer archivos en otros formatos (Excel, SPSS, etc.) y
